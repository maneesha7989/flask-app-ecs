  ...
  stages{
    ...
    stage('Push to Docker'){
      steps{
         ...
         sh "docker image tag flask-app:v1 ${env.DOCKERHUB_CREDENTIALS_USR}/one-tier-flask-app:v2"
         sh "docker push ${env.DOCKERHUB_CREDENTIALS_USR}/one-tier-flask-app:v2"
         }
      }
    }
    stage('done'){
